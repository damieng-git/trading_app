...
# Dashboard Changelog — April 2026

---

## 2026-03-27 — Architecture A (Pullback-A) replaces Swing

**Files:** `strategy.py`, `build_dashboard.py`, `config.json`, `registry.py`, `chart_builder.js`, `templates.py`, `dashboard_screener.js`

Swing strategy removed. Replaced by `arch_a` — a dedicated pullback-in-uptrend engine.

**New engine: `compute_arch_a_position_events()`**
- G1: SMA50W > SMA200W (weekly context; forward-filled to daily if `weekly_df` provided)
- G2: RSI14 < 50 (dip condition)
- G4: MACD histogram crosses above zero (onset trigger)
- G5 exit: Chandelier Exit (`close < HH22 − 3×ATR22`) OR ATR backstop (`close < entry − K×ATR14`, default K=2.5)
- No C4 scale-up; `scaled=False` always
- C3 regime: G1 ∧ G2 ∧ ¬G5 (wider than entry trigger, as expected)

**config.json:** `arch_a` added to `strategy_setups` with `badge_prio: 2`, `badge_label: "A"`. Active on 1D and 1W.

**registry.py:** 3 `IndicatorDef` entries with `strategies=["arch_a"]` — proxy entries for G1/G2/G4 using existing KPI columns (`SuperTrend`, `cRSI`, `CM_Ult_MacD_MFT`).

**scan_list.csv:** Swing removed from unified scan output; arch_a added.

---

## 2026-04-06 — Gate strategy KPI path generalised + build-time validator

**Files:** `chart_builder.js`, `data_exporter.py`, `registry.py`, `CLAUDE.md`, `docs/chart_render_spec.md`, `docs/strategy_pipeline_design.md`

**Problem:** Any `entry_type` other than `polarity_combo` or `stoof` fell through to the generic trend KPI list in `chart_builder.js`. `archAKpiNames`/`archASlice` were dead code — computed but never wired.

**Fix:**
- Removed dead `archAKpiNames`/`archASlice` from `chart_builder.js`
- Added `_gateKpiNames`/`_gateSlice` — reads `strategy_kpis[key]` from the asset for any gate-type strategy
- Wired into `activeStratKpiNames` (breakout filter), `_heatmapOrder` (trend heatmap), `scoreSlice`/`scoreLabel` (score bar)
- Added `console.warn` when generic trend KPI fallback triggers
- `data_exporter.py`: build-time `logger.warning` for any active strategy with empty `strategy_kpis[key]`
- `registry.py` and `CLAUDE.md` updated with authoritative "Adding a new strategy" steps

**Invariant:** any strategy with a non-empty `strategy_kpis[key]` in the asset automatically uses its own KPI list in the heatmap, breakout panel, and score bar — no JS changes required for new gate strategies.

---

## 2026-04-06 — Documentation and audit update

- `docs/strategy_audit.md` — all 17 bugs re-verified against code; statuses updated
- `docs/architecture_audit.md` — all 30 recommendations re-verified; 7 items marked fixed
- `docs/strategy_pipeline_design.md` — Section 5 gap table updated (all P1–P8 closed); Swing removed from engine list
- `docs/chart_render_spec.md` — removed stale `swing→strat_swing` screener map reference
- `docs/pine_to_python_mapping.md` — timeframes updated (4H removed; 2W/1M added)
- `docs/strategy_changes.md` — BUG-S1 marked superseded; P6 gate table updated
- `docs/2-bar sequence test.rtf` — deleted (obsolete scratch notes)
